...
public class HttpRequestBroker extends RequestBroker {
  ...
  public JSONObject request(MushikagoRequest request, ConnectInfo ci) throws RequestException, BrokerException {
    ...
    try {
      ...
      ((AbstractHttpClient) http).setHttpRequestRetryHandler(new DefaultHttpRequestRetryHandler(this.retryCount, false));
      ...
      throw new BrokerException(String.format("%s:%s", e.getClass().getName(), e.getMessage()));
    } catch (IOException e) {
      throw new BrokerException(String.format("%s:%s", e.getClass().getName(), e.getMessage()));
    }
  }
