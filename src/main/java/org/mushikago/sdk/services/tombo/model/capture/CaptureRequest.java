...
public class CaptureRequest extends TomboRequest {
  ...
  public CaptureRequest(String url, ImageFormat fmt, int quality, boolean requireThumbnail, String tag) {
    this(url, fmt, quality, requireThumbnail, new String[] {
      tag
    });
  }
  ...
  public boolean getRequireThumbnail() {
    return this.requireThumbnail;
  }
  ...
  public void setImageFormat(ImageFormat fmt) {
    this.format = fmt;
  }
  ...
  public void setRequireThumbnail(boolean require) {
    this.requireThumbnail = require;
  }
  ...
  public CaptureRequest withDisplayWidth(int displayWidth) {
    CaptureRequest result = this.copy();
    result.setDisplayWidth(displayWidth);
    return result;
  }
  ...
  public String toString() {
    return String.format(
      "url=%s, fmt=%s, quality=%d, requireThumbnail=%b, tags=%s, user_agent=%s, delayTime=%d, displayWidth=%d",
      this.url,
      this.format,
      this.quality,
      this.requireThumbnail,
      this.getOfficialTags(),
      this.userAgent,
      this.delayTime,
      this.displayWidth);
  }
  ...
  public HttpRequestBase toHttpMethod(MushikagoAuth auth, ConnectInfo ci) throws RequestException {
    try {
      ...
      if (null != this.userAgent) {
        requestParams.put("user_agent", ParamUtils.paramEncode(this.userAgent).replaceAll("\\+", "%20"));
      }
